...
const Card: React.FC<CardProps> = ({ card }) => {
  ...
  return (
      <div className='relative group overflow-hidden' onClick={() => router.push(`/movie/${generateSlug(card.title)}?id=${card.id}`)}>
        <Image src={`${imageBaseUrl}${card.poster_path}`} alt={card.title} width={200} height={300} className='rounded-sm w-full h-auto group-hover:transform group-hover:scale-110 transition-transform duration-300'/>
        <span className='px-2 py-1 bg-cta text-white rounded-sm absolute top-4 right-4 font-medium shadow-md'>{card.vote_average.toFixed(1)}</span>
